...
def read_glb(path: Path):
    ...
    doc = json.loads(json_bytes.rstrip(b" \x00").decode("utf-8"))
...
def required_bones(doc):
    ...
    if "VRM" in extensions:
        rows = extensions["VRM"].get("humanoid", {}).get("humanBones", [])
...
def optimize(src: Path, dst: Path, max_size: int = 1024):
    ...
    for index, view in enumerate(views):
        if view.get("buffer", 0) != 0:
            raise ValueError("only single-buffer GLB/VRM files are supported")
        start = view.get("byteOffset", 0)
        length = view["byteLength"]
        piece = old_bin[start : start + length]
        image = image_by_view.get(index)
        if image:
            total_before += len(piece)
            next_piece = optimize_image(piece, image.get("mimeType", ""), max_size)
            total_after += len(next_piece)
    ...
    if not doc.get("buffers"):
        raise ValueError("GLB buffer metadata is missing")
    doc["buffers"][0]["byteLength"] = len(new_bin)

    bones, vrm_version = required_bones(doc)
    required = {
        "hips", "spine", "head",
    ...
    out = bytearray(struct.pack("<4sII", b"glTF", 2, total_length))
    ...
    dst.write_bytes(out)
    ratio = len(out) / src.stat().st_size
    print(
        f"{src.name}: {vrm_version}, {len(bones)} humanoid bones, "
        f"{optimized_count}/{len(images)} textures resized; "
        f"{src.stat().st_size / 1048576:.1f} MiB -> {len(out) / 1048576:.1f} MiB ({ratio:.1%})"
    )
    if total_before:
        print(f"  embedded images: {total_before / 1048576:.1f} MiB -> {total_after / 1048576:.1f} MiB")
